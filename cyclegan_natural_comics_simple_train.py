...
def main():
    ...
    if os.path.exists(checkpoint_file):
        ...
        optimizer_discrim_natural.load_state_dict(
            checkpoint["optimizer_discrim_natural_state_dict"]
        )
        optimizer_discrim_comics.load_state_dict(
            checkpoint["optimizer_discrim_comics_state_dict"]
        )
    ...
    while epoch + 1 <= 100:
        ...
        for i, batch in enumerate(dataloader):
            ...
            valid = torch.Tensor(
                np.ones((real_natural.size(0), *discrim_natural.output_shape))
            ).to(device)
            fake = torch.Tensor(
                np.zeros((real_natural.size(0), *discrim_natural.output_shape))
            ).to(device)
            ...
            # Identity loss
            loss_id_natural = criterion_identity(
                comics2natural(real_natural), real_natural
            )
            loss_id_comics = criterion_identity(
                natural2comics(real_comics), real_comics
            )
            ...
            loss_GAN_comics2natural = criterion_GAN(
                discrim_natural(fake_natural), valid
            )
            ...
            print(
                ...
                i + 1,
                "GANn2c:",
                loss_GAN_natural2comics.item(),
                "GANc2n:",
                loss_GAN_comics2natural.item(),
                "cycle:",
                loss_cycle.item(),
                "identity:",
                loss_identity.item(),
                "Dn:",
                loss_discrim_natural.item(),
                "Dc:",
                loss_discrim_comics.item(),
            )
