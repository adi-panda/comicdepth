def main(lr):
    ...
    class ImageDataset_naturalWithDepth_comicsWithBalloons(Dataset):
        ...
        def __init__(self, unaligned=False):

            self.list_comics = []
            with open(os.path.join("data/dcm_cropped", "train.txt")) as file:
                content = file.read().split("\n")
                self.list_comics += [
                    os.path.join("data/dcm_cropped/images", x + ".jpg")
                    for x in content
                    if x != ""
                ]
        ...
        def __getitem__(self, index):
            index2 = (
                random.randrange(0, len(self.list_comics)) if self.unaligned else index
            )
            natural_name = self.list_natural[index % len(self.list_natural)]
            comics_name = self.list_comics[index2 % len(self.list_comics)]
            balloons_name = comics_name.replace(
                "dcm_cropped/images", "dcm_cropped/balloons427"
            ).replace(".jpg", "_originalsize.png")

            depth_name = natural_name.replace(
            ...
            if image_natural_depth.ndim != 2:
                image_natural_depth = cv2.cvtColor(
                    image_natural_depth, cv2.COLOR_BGR2GRAY
                )
            ...
            item_natural, item_natural_depth = get_random_crop(
                item_natural, item_natural_depth, 384, 384
            )
            item_comics, item_balloons = get_random_crop(
                item_comics, item_balloons, 384, 384
            )
    ...
    class GeneratorResNet(nn.Module):
        def __init__(
            self, input_shape=(3, 256, 256), out_channels=3, num_residual_blocks=8
        ):
    ...
    natural2comicsNoB = GeneratorResNet(
        input_shape=(3, 384, 384), out_channels=3, num_residual_blocks=8
    )
    comicsNoB2comics = GeneratorResNet(
        input_shape=(7, 384, 384), out_channels=3, num_residual_blocks=8
    )
    ...
    checkpoint_file = os.path.join(
        "models/trained/cyclegan_natural_comics_without_text", "99.pth"
    )
    ...
    checkpoint_file = os.path.join(
        "models/trained/cyclegan_natural_comics_add_text", "99.pth"
    )
    checkpoint = torch.load(checkpoint_file)
    ...
    if os.path.exists(checkpoint_file):
        ...
        optimizer_comics2depth.load_state_dict(
            checkpoint["optimizer_comics2depth_state_dict"]
        )
    ...
    while epoch + 1 <= 100:
        ...
        for i, batch in enumerate(dataloader):
            ...
            loss = criterion_depth(
                pred_depth * (1 - real_balloons),
                real_natural_depth * (1 - real_balloons),
            )
            ...
            print("epoch:", epoch + 1, "i:", i + 1, "loss:", loss.item())
