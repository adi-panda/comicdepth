            ...
            if child.tag == "{http://www.w3.org/2000/svg}svg":
                if child.attrib["class"] == "Page":
                    ...
                    for child2 in child:
                        ...
                        points = [[int(x) for x in pt.split(",")] for pt in points]
...
def generate_evaluation() -> None:
    ...
    for line in lines:
        img_name = line[:-1]
        depth_filename = os.path.join("data/dcm_cropped/depth", img_name + ".txt")
        ...
        if os.path.exists(depth_filename):
            ...
            img_full = cv2.resize(img_full, (int(w * img.height() / h), img.height()))
            img_full = Image.fromarray(img_full)
            img_full = ImageTk.PhotoImage(img_full)

            canvas = tkinter.Canvas(fen, height=img.height(), width=img.width())
            ...
            def paint(event):
                ...
                for pt in depthOrderingStatus.annotation:
                    l1, l2, x, y = pt
                    canvas.create_line(x - 5, y, x + 5, y, fill=colors[l1], width=2)
                    canvas.create_line(x, y - 5, x, y + 5, fill=colors[l1], width=2)
                    canvas.create_text(
                        x,
                        y,
                        fill="darkblue",
            ...
            def add_point(event):
                new = (
                    depthOrderingStatus.inter_object_level,
